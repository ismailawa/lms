...
const PricingCard: React.FC<PricingCardProps> = ({ title, price, features, description, isCurrent, onSelect }) => {
  return (
    <div className={`p-3 w-[250px] rounded-2xl flex flex-col justify-between ${isCurrent ? 'bg-[#00550E] text-white ' : 'bg-[#FFFFFF80] text-black mt-3'}`}>
      ...
      <button 
        onClick={onSelect} 
        className={`w-full py-2 mt-4 text-xs font-bold ${isCurrent ? 'bg-white hover:bg-[#fffffff2] text-green-500' : 'bg-[#0D9422] hover:bg-[#0D9533] text-white'} rounded-2xl`}>
